# services/models/classification/Image/VGG16.py
...
def train_vgg16(data_dir, categories, epochs=50, batch_size=16, learning_rate=0.001, eval_metric="accuracy", task_id=None, task_manager=None):
    """
    训练 VGG16 模型
    ...
    log_filename = f"traininglogs/{task_id}_{current_time}_vgg16.log" if task_id else f"traininglogs/{current_time}_vgg16.log"
    
    # 打开日志文件
    log_file = open(log_filename, 'w')
    
    # 记录训练模型和超参数
    log_file.write(f"Model: VGG16\n")
    ...
    try:
        ...
        transform = transforms.Compose([
            transforms.Resize((224, 224)),  # VGG16 要求输入大小为 224x224
            transforms.ToTensor(),
        ...
        # 由于 VGG16 较大，可以选择只训练分类器部分的参数，冻结特征提取器
        # 这样可以加快训练速度并减少过拟合的风险
        ...
        for epoch in range(epochs):
            ...
            print(f"[VGG16] Epoch {epoch}: Loss={epoch_loss:.4f}, {eval_metric.capitalize()}={epoch_metric:.4f}")
