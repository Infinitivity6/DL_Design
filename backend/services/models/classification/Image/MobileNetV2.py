# services/models/classification/Image/MobileNetV2.py
...
def train_mobilenetv2(data_dir, categories, epochs=50, batch_size=16, learning_rate=0.001, eval_metric="accuracy", task_id=None, task_manager=None):
    """
    训练 MobileNetV2 模型
    ...
    log_filename = f"traininglogs/{task_id}_{current_time}_mobilev2.log" if task_id else f"traininglogs/{current_time}_mobilev2.log"
    
    # 打开日志文件
    log_file = open(log_filename, 'w')
    
    # 记录训练模型和超参数
    log_file.write(f"Model: MobileNetV2\n")
    ...
    try:
        # 定义数据预处理
        # MobileNetV2 通常使用 224x224 的输入尺寸
        ...
        # MobileNetV2 的一个优点是参数少，可以选择训练所有层
        # 但也可以只训练分类器部分，这里我们选择训练所有层
        optimizer = optim.Adam(model.parameters(), lr=learning_rate)
        
        # 可选：学习率调度器，随着训练进行降低学习率
        ...
        for epoch in range(epochs):
            ...
            print(f"[MobileNetV2] Epoch {epoch}: Loss={epoch_loss:.4f}, {eval_metric.capitalize()}={epoch_metric:.4f}")
        ...
        print(f"[MobileNetV2] Error: {str(e)}")
